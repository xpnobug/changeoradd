# changeoradd 集成说明
项目路径：/openclaw/ui/src/ui/changeoradd
### /openclaw/extensions 目录复制粘贴放入 /workspace-editor 文件

也可以直接使用[change-file](change-file) 目录下的直接覆盖对应文件
...
extensions/workspace-editor/workspace-files.ts
...
| `ui/src/ui/app-settings.ts` | +1 | 添加标签页刷新逻辑（已修复重复代码） |
